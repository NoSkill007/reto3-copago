...
let catalog, caseId, awaitingReply = false, caseToken = 0, selectedPlan;
const empty = $('results').innerHTML;
const sourceLabel = source => source==='qvac' ? 'Respuesta generada por QVAC local' : 'Modo de reglas · sin IA';

async function status(){
 try{
  const s=await fetch('/api/status').then(r=>r.json());
  $('status').textContent=s.connected?'QVAC conectado · IA local':'QVAC desconectado · Reglas de demo';
  $('device').textContent=s.device;
  $('status-dot').classList.toggle('connected', s.connected);
 }catch{
...
function renderPlanToggle(){
 $('plan-toggle').innerHTML = catalog.plans.map(p => `<button type="button" class="plan-card" data-plan="${escape(p.id)}" aria-pressed="${p.id===selectedPlan}"><span class="plan-name">${escape(p.name)}</span><span class="plan-detail">${escape(p.description)}</span></button>`).join('');
}
...
async function beginCase(){
 ...
 try{
  const response=await fetch('/api/case',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({plan:selectedPlan})});
...
$('plan-toggle').addEventListener('click', e => {
 const button = e.target.closest('.plan-card');
...
try{
 ...
 renderPlanToggle();
 await beginCase();
}catch{$('error').textContent='No se pudo cargar el catálogo. Recarga la página.';}
